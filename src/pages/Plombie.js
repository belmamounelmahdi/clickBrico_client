...
function Plombie () {
    ...
    useEffect(() => {
        axios.get(`${API_URL}/service-plombie`)
        .then(res => {
    ...
    return (
    <div>
        {jobberProfiles.length === 0 ?<div className='flex flex-col h-screen items-center mt-24 space-y-10'> <Loading /> <h1 className='font-bold text-lg'>Désolé ce service ne dispose pas des prestataires</h1></div> : (
        ...
        <button onClick={() => handleSelectProvider(ele.email)} className='bg-sky-600 hover:bg-sky-500 hidden md:inline p-2 text-white'>
            Envoyer un message
        </button>
            </div>
        ))}
